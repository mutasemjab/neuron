...
@push('styles')
    <style>
        ...
        [dir="rtl"] .book-choice-cta svg {
            transform: scaleX(-1);
        }

        .book-choice-card:hover .book-choice-cta svg {
            transform: translateX(4px);
        }

        [dir="rtl"] .book-choice-card:hover .book-choice-cta svg {
        ...
        .book-page-card-head .bpc-icon svg {
            width: 24px;
            height: 24px;
        }

        .book-page-card-head h2 {
            font-size: 1.3rem;
            color: var(--ink);
            margin-bottom: 6px;
        }

        .book-page-card-head p {
            color: var(--ink-soft);
            font-size: .9rem;
            margin: 0;
        }
        ...
        .cal-legend i.dot-disabled {
            background: #e5e9e8;
        }

        .cal-legend i.dot-selected {
            background: var(--teal);
        }
            ...
            <div class="book-step" data-step="domestic">
                ...
                <div class="book-page-card">
                    <div class="book-page-card-head">
                        <div class="bpc-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2">
                                <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
                                <circle cx="12" cy="10" r="3" />
                            </svg></div>
                    ...
                    @if (sett('booking_page.price'))
                        <div class="book-price-badge">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                <circle cx="12" cy="12" r="10" />
                                <path
                                    d="M12 6v12M15 9.5c0-1.4-1.3-2.5-3-2.5s-3 1.1-3 2.5 1.3 2 3 2 3 .6 3 2-1.3 2.5-3 2.5-3-1.1-3-2.5" />
                            </svg>
                            <div>
                                <span
...
@push('scripts')
    <script>
        ...
        (function() {
            ...
            function render() {
                daysGrid.innerHTML = '';
                monthLabel.textContent = viewDate.toLocaleDateString(isAr ? 'ar-JO' : 'en-US', {
                    month: 'long',
                    year: 'numeric'
                });
            ...
            function selectDate(date) {
                ...
                triggerLbl.textContent = date.toLocaleDateString(isAr ? 'ar-JO' : 'en-US', {
                    weekday: 'long',
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric'
        ...
        @if (sett_raw('booking_page.price_amount'))
            (function() {
                ...
                function showResult(success) {
                    ...
                    resultEl.innerHTML = '<p style="color:' + (success ? 'var(--teal-deep)' : '#dc2626') + '">' + (
                        success ? cfg.i18n.success : cfg.i18n.failed) + '</p>';
                ...
                payBtn.addEventListener('click', async () => {
                    ...
                    try {
                        ...
                        const transientToken = await up.show({
                            containers: {
                                paymentSelection: '#consultPaymentForm'
                            }
                        });
