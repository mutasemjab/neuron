...
<header id="header">
    <div class="topbar">
        <div class="wrap">
            <div class="topbar-l">
                <a href="tel:{{ sett_raw('contact.phone') }}" class="hotline-link"><span class="hotline-pulse"></span><svg
                    ...
                    </svg><span>{{ sett('contact.hotline_note') }}:
                        <strong dir="ltr">{{ sett_raw('contact.phone') }}</strong></span></a>
                <a href="mailto:{{ sett_raw('contact.email') }}"><svg viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2">
                        <rect x="2" y="4" width="20" height="16" rx="2" />
                        <path d="m22 7-10 5L2 7" />
                    </svg><span>{{ sett_raw('contact.email') }}</span></a>
            </div>
            <div class="topbar-r">
                <a href="{{ route('home') }}#locations"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2">
                        <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
                        <circle cx="12" cy="10" r="3" />
                    </svg><span>{{ sett_raw('contact.branches_count') }} {{ __('front.topbar_branches') }}</span></a>
                ...
                <a href="{{ \Mcamara\LaravelLocalization\Facades\LaravelLocalization::getLocalizedURL(app()->getLocale() === 'ar' ? 'en' : 'ar') }}"
                    rel="alternate">{{ __('front.lang_switch') }}</a>
            </div>
        </div>
    </div>
    ...
    <nav class="nav">
        <div class="wrap">
            <a href="{{ route('home') }}" class="logo" aria-label="{{ sett('identity.site_name') }}">
                ...
                <span class="logo-txt"><img src="{{ asset_v('assets_front/images/logonav.png') }}" alt=""></span>
            </a>
            ...
            <div class="nav-cta">
                <a href="{{ route('booking.page') }}" class="btn btn-primary"><span>{{ __('front.book_now') }}</span>
                ...
                <button class="burger" id="burger"
                    aria-label="{{ __('front.nav_menu') }}"><span></span><span></span><span></span></button>
            </div>
        </div>
    </nav>
</header>
