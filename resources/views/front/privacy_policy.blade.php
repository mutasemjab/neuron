...
@section('title', sett('legal.privacy_title') . ' — ' . sett('identity.site_name'))
@section('meta_description', sett('legal.privacy_title'))
  ...
  <div class="wrap">
    <div class="pp-wrap">
      ...
      <div class="pp-crumbs">
        ...
        <span>/</span>
        <span>{{ sett('legal.privacy_title') }}</span>
      </div>

      <h1 class="pp-title">{{ sett('legal.privacy_title') }}</h1>
      <p class="pp-updated">{{ app()->getLocale() === 'ar' ? 'آخر تحديث' : 'Last updated' }}: {{ now()->translatedFormat('d M Y') }}</p>
      <div class="pp-content">
        @include('front._legal_content', ['content' => sett('legal.privacy_content')])
      </div>
