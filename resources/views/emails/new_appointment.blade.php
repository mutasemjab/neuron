      ...
      <div style="padding:28px;">
        <table style="width:100%;border-collapse:collapse;font-size:14px;color:#1a2e2b;">
          ...
          <tr>
            ...
            <td style="padding:10px 0;border-bottom:1px solid #eee;font-weight:bold" dir="ltr">{{ $appointment->phone }}</td>
          </tr>
          <tr>
            <td style="padding:10px 0;border-bottom:1px solid #eee;color:#6b7d7a">نوع الحجز</td>
            <td style="padding:10px 0;border-bottom:1px solid #eee">{{ $appointment->booking_type === 'international' ? 'خارج الأردن' : 'داخل الأردن' }}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding:10px 0;border-bottom:1px solid #eee">{{ $appointment->doctor->name_ar ?? '—' }}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding:10px 0">{{ $appointment->notes ?: '—' }}</td>
          </tr>
        </table>

        @if($appointment->attachment)
        <p style="margin:20px 0 0">
          <a href="{{ $appointment->attachment_url }}" style="color:#2a807d;font-weight:bold;text-decoration:none">📎 عرض الملفات المرضية المرفقة</a>
        </p>
        @endif
